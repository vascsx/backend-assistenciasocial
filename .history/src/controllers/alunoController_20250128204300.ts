import { FastifyReply, FastifyRequest } from 'fastify';import { connectMySQL } from '../db';import mysql from 'mysql2/promise';interface Aluno {  nome: string;  matricula: string;  curso: string;  email: string;}
...
export async function createAluno(request: FastifyRequest, reply: FastifyReply) {
  try {
    ...
    // Caso ocorra o erro de duplicação devido a uma violação da chave única, tratamos separadamente
