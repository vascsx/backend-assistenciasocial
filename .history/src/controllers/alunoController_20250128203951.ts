...
export async function createAluno(request: FastifyRequest, reply: FastifyReply) {
  try {
    ...
    // Verificando se existe aluno com a mesma matrícula
    if ((existingAluno as any[]).some(aluno => aluno.matricula === matricula)) {
      return reply.status(400).send({ message: 'Já existe aluno cadastrado com essa matrícula' });
    }

    // Verificando se existe aluno com o mesmo email
    if ((existingAluno as any[]).some(aluno => aluno.email === email)) {
