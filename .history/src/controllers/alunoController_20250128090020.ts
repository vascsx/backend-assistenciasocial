...
export async function createAluno(request: FastifyRequest, reply: FastifyReply) {
  try {
    ...
    // Realizando a inserção na tabela alunos
    const [result] = await connection.execute(
      'INSERT INTO alunos (nome, matricula, curso) VALUES (?, ?, ?)',
      [nome, matricula, curso]
    );

    // O resultado da execução será um array de ResultSetHeader
    const insertId = (result as mysql.ResultSetHeader).insertId;

    return reply.status(201).send({ id: insertId, nome, matricula, curso });
  } catch (error) {
