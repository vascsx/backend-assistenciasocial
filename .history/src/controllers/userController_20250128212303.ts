  ...
  try {
    ...
    if (!user) {
      return reply.status(404).send({
        message: `Nenhum usuário encontrado com o ${isEmail ? 'email' : 'matrícula'} fornecido.`,
      });
    }
    ...
    if (!isPasswordValid) {
      return reply.status(401).send({ message: 'Senha incorreta. Verifique e tente novamente.' });
    }
